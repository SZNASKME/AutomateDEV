    ...
    with open(jil_filepath, "rt") as jil:
        ...
        for linesInJill in jilLines:
            ...
            if "insert_job:" in linesInJill:
                if oneJob:  # Add previous job if exists
                    jil_in_array.append(oneJob)
                    dynamic_fieldnames.update(oneJob.keys())
                ...
                if linesInJill != "\n" and "/* ----" not in linesInJill and linesInJill != "":
                    if "description:" in linesInJill:
                        ...
                        if ":" in linesInJill:
                            ...
                            field_name = str(spli[0]).strip()
                            field_value = str(spli[1]).strip().replace("\"","")
                            oneJob[field_name] = field_value
        ...
        # Combine base fieldnames with dynamic ones found in file
        # Base fields come first, then additional dynamic fields
        base_fields_set = set(jil_base_fieldnames)
        additional_fields = dynamic_fieldnames - base_fields_set
        final_fieldnames = jil_base_fieldnames + sorted(additional_fields)
        
        print(f"\nTotal jobs: {count}")
        print(f"Base fields: {len(jil_base_fieldnames)}")
        print(f"Additional dynamic fields: {len(additional_fields)}")
        print(f"Total fields: {len(final_fieldnames)}")
        if additional_fields:
            print(f"New fields found: {sorted(additional_fields)}")
    
    return jil_in_array, final_fieldnames
...
def main():
    ...
    jil_in_array, dynamic_fieldnames = readJILFile(jil_path)
    ...
    # Use dynamic fieldnames instead of predefined ones
    df_job_ori = pd.DataFrame(jil_in_array, columns=dynamic_fieldnames)
    if jil_except_path != 'n':
        df_job_except = pd.DataFrame(jil_in_array_except, columns=dynamic_fieldnames)
        
    if files and STDCAL_FILENAME in files.keys():
        print("Parsing Standard Calendar Data...")
        stdcel_parsed_data = stdcelParseData(files[STDCAL_FILENAME])
        df_stdcal = pd.DataFrame(stdcel_parsed_data)
    if files and EXTCAL_FILENAME in files.keys():
        print("Parsing Exclude Calendar Data...")
