...
FILTER_COLUMN = "Use / Not Use"
FILTER_VALUE_1 = "Use"
FILTER_VALUE_2 = "Migrated"


CREATE_TASK_FLAG = True
CREATE_TRIGGER_FLAG = True
...
def restructureTriggerConfigs(trigger_configs_list):
    ...
    def getTriggerDetails(trigger_configs_list):
        ...
        for value in trigger_configs_list:
            ...
            trigger_name = f"{trigger_task_name}_{SUFFIX_WORKFLOW_NAME}"
            trigger_task_name = f"{trigger_task_name}_{SUFFIX_WORKFLOW_NAME}"
    ...
    if len(analyze_trigger_type) == 0:
        ...
        for task_detail in trigger_details_list:
            ...
            if task_detail['type'] == "Daily":
                ...
                new_trigger_configs['description'] = f"{description_str}"
            ...
            if task_detail['type'] == "One Time":
                ...
                new_trigger_configs['description'] = f"[Manual]"
                new_trigger_configs_list.append(new_trigger_configs)
                manual_trigger_count += 1
    else:
        trigger_not_created_list.append({
...
def createTriggerFromAPI(df, column_mapping):
    ...
    def renameDuplicateValuesInList(configs_list, original_key, append_key, is_duplicate_name=False):
        ...
        for config in configs_list:
            ...
            config[original_key] = f"{new_value}"
...
def main():
    ...
    df_filtered = df[df[FILTER_COLUMN].isin([FILTER_VALUE_1, FILTER_VALUE_2])]
    df_trigger_log = createTriggerFromAPI(df_filtered, trigger_column_mapping)
